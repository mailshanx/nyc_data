...
class StringEncoder:
    ...
    def __init__(self):
        self.sI1 = StringIndexer(inputCol="ratecode_id", outputCol="ratecode_index", handleInvalid="keep")
        self.sI2 = StringIndexer(inputCol="vendor_id", outputCol="vendor_index", handleInvalid="keep")
        self.sI3 = StringIndexer(inputCol="payment_type", outputCol="payment_type_index", handleInvalid="keep")
        self.sI4 = StringIndexer(inputCol="store_and_fwd_flag", outputCol="store_and_fwd_flag_index",
                                 handleInvalid="keep")
...
class FormEncoder:
    ...
    def __init__(self, formula="tip_amount ~ passenger_count + fare_amount + vendor_index + ratecode_index \
                        + trip_duration_m + store_and_fwd_flag_index + trip_type + pu_location_id + do_location_id \
                         + trip_distance"):
        self.reg_formula = RFormula(formula=formula)

        self.feature_indexer = VectorIndexer(inputCol="features", outputCol="indexed_features",
                                             handleInvalid="keep", maxCategories=270)
